// University Programs Data Structure (ESM)
...
export const UNIVERSITY_PROGRAMS = {
  ...
  [PROGRAM_CATEGORIES.SCIENCE]: [
    {
      ...
      duration: '3 years',
      hasLateralEntry: false,
      specializations: ['Biotechnology', 'Microbiology', 'Applied Psychology'],
    },
    {
      degree: 'M.Sc',
  ...
  [PROGRAM_CATEGORIES.MANAGEMENT]: [
    ...
    },
    {
      ...
      duration: '2 years',
      hasLateralEntry: false,
      specializations: ['Marketing', 'Finance', 'Human Resources', 'Agri-Business', 'Operations Management', 'Information Technology'],
    },
    {
      degree: 'B.Com (Hons.)',
      ...
      duration: '3 years',
      hasLateralEntry: false,
      specializations: ['Taxation', 'E-Commerce', 'Banking & Finance', 'Accounting'],
    },
...
export const getProgramsByCategory = (category) => {
  const val = getCategoryValue(category);
  return val ? UNIVERSITY_PROGRAMS[val] || [] : [];
};
...
export const validateProgramSelection = (category, degree, specialization) => {
  const programs = getProgramsByCategory(category);
  const program = programs.find((p) => p.degree === degree);

  if (!program) return { valid: false, message: 'Invalid program selected' };

  if (program.specializations.length > 0 && !specialization)
    return { valid: false, message: 'Specialization is required for this program' };

  if (specialization && program.specializations.length > 0) {
    if (!program.specializations.includes(specialization))
      return { valid: false, message: 'Invalid specialization for selected program' };
  }
